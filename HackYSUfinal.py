...
print('This was developed by Austin Vingle, Andrew Trebella, Jim Galioto, Alisha Dodge and Victoria Thomas. With help from Brian Kost at Hack YSU 2017.')
print("Type 'help'")
def areaOne(userInput):
	...
	# 	return 106
		...
		print('Instantly restored 10 health.')
...
def shop(userInput):
	if userInput.lower() == 'exit':
		...
		return 4.1
	elif character.rightarm == ['shovel'] and userInput.lower() == 'sell shovel':
...
def blacksmith(userInput):
	if userInput.lower() == 'exit':
		...
		return 4.2
	elif character.gold >= 15 and userInput.lower() == 'buy sword':
...
def areaFive(userInput):
	if userInput.lower() == "move north":
		...
		print("A voice echos, You must go into the dungeon to find the eight peices of the trinity to activate the portal")
		return 6
	elif userInput.lower() == 'help':
		for element in character.help:
			print(element)
...
def areaSeven(userInput):
	...
	elif character.inventory == ['shard of glass'] and userInput.lower() == 'use shard of glass':
...
def combat_staircase_jim(userInput):
	incombat = True
	if character.gold == 1 and userInput.lower() == 'use gold':
		print('You attempt to bribe Jim with your remaining 1 gold piece.')
		print('Being as greedy as Jim is, and since Dittirch is stingy with his pay, Jim accepts with no hesitation.')
		return 10
...
def final_combat(userInput):
	...
	while(incombat == True):
		if (character.health <= 0):
			...
			dittrich.health -= (character.power - dittrich.defense)
			...
			if(dittrich.health <= 0):
				...
				print('You accend into the Hall of Glory')
